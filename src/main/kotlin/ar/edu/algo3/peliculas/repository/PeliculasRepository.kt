...
interface PeliculasRepository : Neo4jRepository<Pelicula, Long>  {
    ...
    @Query("MATCH (pelicula:Movie)<-[actuo_en:ACTED_IN]-(persona:Person) WITH pelicula, actuo_en, persona ORDER BY ID(persona) WHERE ID(pelicula) = \$id RETURN pelicula, COLLECT(actuo_en), COLLECT(persona) LIMIT 1")
